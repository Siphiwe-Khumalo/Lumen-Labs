...
- **Domain and SEO:** the current canonical URL is `https://lumen-labs.onrender.com/`. Update `index.html`, `public/robots.txt`, and `public/sitemap.xml` if a custom domain is configured.
- **Form delivery:** Render's static site plan is static hosting only. Set `VITE_FORM_ENDPOINT` to a real HTTPS form endpoint before relying on the enquiry form in production.
